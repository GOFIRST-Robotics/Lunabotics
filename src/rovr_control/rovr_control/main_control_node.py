...
class MainControlNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter(
            ...
            0.12)  # Measured in Duty Cycle (0.0-1.0)
        self.declare_parameter(
            "digger_lift_manual_power_up",
            0.5)  # Measured in Duty Cycle (0.0-1.0)
        self.declare_parameter(
            "tilt_digging_start_position",
            125.0)  # Measured in encoder counts
        ...
        self.digger_chain_power = self.get_parameter(
            "digger_chain_power").value
        self.digger_lift_manual_power_down = self.get_parameter(
            "digger_lift_manual_power_down").value
        self.digger_lift_manual_power_up = self.get_parameter(
            "digger_lift_manual_power_up").value
        self.autonomous_field_type = self.get_parameter(
            "autonomous_field_type").value
        self.tilt_digging_start_position = self.get_parameter(
            "tilt_digging_start_position").value
        self.dumper_power = self.get_parameter("dumper_power").value
        self.DIGGER_SAFETY_ZONE = self.get_parameter(
            "DIGGER_SAFETY_ZONE").value
        ...
        self.get_logger().info("digger_chain_power has been set to: " +
                               str(self.digger_chain_power))
        self.get_logger().info("digger_lift_manual_power_down has been set to: " +
                               str(self.digger_lift_manual_power_down))
        self.get_logger().info("digger_lift_manual_power_up has been set to: " +
                               str(self.digger_lift_manual_power_up))
        self.get_logger().info("autonomous_field_type has been set to: " +
                               str(self.autonomous_field_type))
        self.get_logger().info("tilt_digging_start_position has been set to: " +
                               str(self.tilt_digging_start_position))
        self.get_logger().info("dumper_power has been set to: " + str(self.dumper_power))
        self.get_logger().info("DIGGER_SAFETY_ZONE has been set to: " +
                               str(self.DIGGER_SAFETY_ZONE))
        ...
        self.field_calibrated_handle: ClientGoalHandle = ClientGoalHandle(
            None, None, None)
        self.auto_dig_handle: ClientGoalHandle = ClientGoalHandle(
            None, None, None)
        self.auto_offload_handle: ClientGoalHandle = ClientGoalHandle(
            None, None, None)
    ...
    async def auto_dig_nav_offload_sequence(self) -> None:
        ...
        if self.auto_dig_nav_offload_handle.status != GoalStatus.STATUS_EXECUTING:
            if not self.act_auto_dig_nav_offload.wait_for_server(
                    timeout_sec=1.0):
            ...
            if not self.auto_dig_nav_offload_handle.accepted:
                ...
                return
            self.auto_dig_nav_offload_handle.get_result_async(
    ...
    async def auto_offload_sequence(self) -> None:
        ...
        if self.auto_offload_handle.status != GoalStatus.STATUS_EXECUTING:
            ...
            self.auto_offload_handle.get_result_async(
            ).add_done_callback(self.get_result_callback)
            self.state = states["Autonomous"]
    ...
    async def calibrate_field_coordinates(self) -> None:
        ...
        if self.field_calibrated_handle.status != GoalStatus.STATUS_EXECUTING:
            ...
            if not self.field_calibrated_handle.accepted:
                ...
                return
            self.field_calibrated_handle.get_result_async(
            ).add_done_callback(self.get_result_callback)
            self.state = states["Autonomous"]
    ...
    async def joystick_callback(self, msg: Joy) -> None:
        ...
        if self.state == states["Teleop"]:
            ...
            # Forward power
            forward_power = msg.axes[bindings.RIGHT_JOYSTICK_VERTICAL_AXIS] * \
                self.max_drive_power
            # Turning power
            turning_power = msg.axes[bindings.LEFT_JOYSTICK_HORIZONTAL_AXIS] * \
                self.max_turn_power
            self.drive_power_publisher.publish(
                Twist(
                    linear=Vector3(
                        x=forward_power), angular=Vector3(
                        z=turning_power)))
            ...
            if msg.buttons[bindings.B_BUTTON] == 1 and buttons[bindings.B_BUTTON] == 0:
                self.cli_dumper_stop.call_async(
                    Trigger.Request())  # Stop whatever the dumper is doing
            ...
            if msg.buttons[bindings.Y_BUTTON] == 1 and buttons[bindings.Y_BUTTON] == 0:
                self.cli_big_agitator_toggle.call_async(
                    Trigger.Request())  # Toggle the agitator motor
