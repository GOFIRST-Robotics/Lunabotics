        ...
        if msg.buttons[START_BUTTON] == 1 and buttons[START_BUTTON] == 0:
            ...
            if self.camera_view_toggled:  # Start streaming /dev/front_webcam on port 5000
                ...
                self.front_camera = subprocess.Popen(
                    'gst-launch-1.0 v4l2src device=/dev/front_webcam ! "video/x-raw,width=640,height=480,framerate=15/1" ! nvvidconv ! "video/x-raw(memory:NVMM),format=NV12" ! nvv4l2av1enc bitrate=200000 ! "video/x-av1" ! udpsink host=192.168.1.110 port=5000',
                    shell=True,
                ...
                self.back_camera = subprocess.Popen(
                    'gst-launch-1.0 v4l2src device=/dev/back_webcam ! "video/x-raw,width=640,height=480,framerate=15/1" ! nvvidconv ! "video/x-raw(memory:NVMM),format=NV12" ! nvv4l2av1enc bitrate=200000 ! "video/x-av1" ! udpsink host=192.168.1.110 port=5000',
                    shell=True,
                    preexec_fn=os.setsid,
                )
