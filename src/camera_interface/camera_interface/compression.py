...
class VideoCompressionNode(Node):
    def __init__(self):
        ...
        # Conversion Logic
        try:
            # If the string is just a number (e.g., "0"), convert to int
            if str(self.device_id).isdigit():
                self.camera_source = int(self.device_id)
            else:
                # If it's a path (e.g., "/dev/video0"), keep it as a string
                self.camera_source = str(self.device_id)
    ...
    def initalize_parameters(self):
        # Define the parameter descriptor to specify the type
        topic_parameter_descriptor = ParameterDescriptor(
            type=ParameterType.PARAMETER_STRING,
            description='A wecam topic to publish to.'
        )
        
        camera_id_parameter_descriptor = ParameterDescriptor(
            type=ParameterType.PARAMETER_STRING,
            description='A wecam id to read from.'
        )

        self.declare_parameter(
            'topic_name',
            value='',
            descriptor=topic_parameter_descriptor
        )
        
        self.declare_parameter(
            'device_id',
            value='0',
            descriptor=camera_id_parameter_descriptor
        )
