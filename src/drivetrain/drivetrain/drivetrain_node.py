...
class DrivetrainNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("EXTENSION_SAFETY_ZONE", 0)  # Measured in potentiometer units (0 to 1023)
        ...
        self.extension_sub = self.create_subscription(Float32, "extension_pos", self.extension_callback, 10)
        ...
        self.get_logger().info("EXTENSION_SAFETY_ZONE has been set to: " + str(self.EXTENSION_SAFETY_ZONE))
    ...
    def drive(self, forward_power: float, turning_power: float) -> None:
        ...
        if (
            (self.extension_pos is None or self.extension_pos > self.EXTENSION_SAFETY_ZONE)
