...
CONVEYORSIZEX = 0.5  # width of the conveyor in meters
CONVEYORSIZEY = 0.5  # length of the conveyor in meters
...
POLLRATE = 0.5  # Wait time between each distance check (in seconds)
...
class ros_check_load(Node):
    ...
    def publish_distance(self):
        try:
            ...
            percentFOVx = (
                perceptionChangeX / 85.2
            )  # compare the degrees of vision it would occupy to the FOV of the realsense
            percentFOVy = perceptionChangeY / 58.5

            change_x = (
                self.img_width * percentFOVx
            ) / 2  # get the number of pixels that it's occupying. divide by 2 so it's a +- situation
