...
class ConveyorNode(Node):
    ...
    def set_power(self, drum_belt_power: float, conveyor_power: float) -> None:
        ...
        self.running = True
        drum_belt_power = min(max(-1.0, drum_belt_power), 1.0)  # Clamp the power between -1.0 and 1.0
        conveyor_power = min(max(-1.0, conveyor_power), 1.0)  # Clamp the power between -1.0 and 1.0
